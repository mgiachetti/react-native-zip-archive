...
+ (BOOL)unzipFileAtPath:(NSString *)path toDestination:(NSString *)destination overwrite:(BOOL)overwrite password:(nullable NSString *)password error:(NSError **)error delegate:(nullable id<RNSSZipArchiveDelegate>)delegate
{
...
+ (BOOL)unzipFileAtPath:(NSString *)path
     ...
     preserveAttributes:(BOOL)preserveAttributes
              overwrite:(BOOL)overwrite
               password:(NSString *)password
                  error:(NSError **)error
               delegate:(id<RNSSZipArchiveDelegate>)delegate
        progressHandler:(void (^)(NSString *entry, unz_file_info zipInfo, long entryNumber, long total))progressHandler
      completionHandler:(void (^)(NSString *path, BOOL succeeded, NSError *error))completionHandler
{
    // Begin opening
    ...
    if (zip == NULL)
    {
        ...
        NSError *err = [NSError errorWithDomain:@"RNSSZipArchiveErrorDomain" code:-1 userInfo:userInfo];
        if (error)
        {
            *error = err;
        }
    ...
    if (unzGoToFirstFile(zip) != UNZ_OK)
    {
        ...
        NSError *err = [NSError errorWithDomain:@"RNSSZipArchiveErrorDomain" code:-2 userInfo:userInfo];
        if (error)
        {
            *error = err;
        }
    ...
    if (success && preserveAttributes) {
        ...
        for (NSDictionary * d in directoriesModificationDates) {
            if (![[NSFileManager defaultManager] setAttributes:@{NSFileModificationDate: d[@"modDate"]} ofItemAtPath:d[@"path"] error:&err]) {
                NSLog(@"[RNSSZipArchive] Set attributes failed for directory: %@.", d[@"path"]);
            }
            if (err) {
    ...
    if (crc_ret == UNZ_CRCERROR)
    {
        ...
        retErr = [NSError errorWithDomain:@"RNSSZipArchiveErrorDomain" code:-3 userInfo:userInfo];
    }
...
{
    return [RNSSZipArchive createZipFileAtPath:path withFilesAtPaths:paths withPassword:nil];
}
+ (BOOL)createZipFileAtPath:(NSString *)path withContentsOfDirectory:(NSString *)directoryPath{
    return [RNSSZipArchive createZipFileAtPath:path withContentsOfDirectory:directoryPath withPassword:nil];
}

+ (BOOL)createZipFileAtPath:(NSString *)path withContentsOfDirectory:(NSString *)directoryPath keepParentDirectory:(BOOL)keepParentDirectory{
    return [RNSSZipArchive createZipFileAtPath:path withContentsOfDirectory:directoryPath keepParentDirectory:keepParentDirectory withPassword:nil];
}
